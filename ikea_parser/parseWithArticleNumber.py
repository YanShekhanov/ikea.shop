...
def parseOneProductInformationWithArticleNumber(article_number, parse_complementary=True):
    ...
    #проверка на наличие артикула в БД
    '''try:
        ...
        if subcategory_status:
            existed_product = Product.objects.get(article_number=product_article,
                                                  subcategory=foreign_key_query)
        # если продукт находится в под подкатегории
        elif sub_subcategory_status:
            existed_product = Product.objects.get(article_number=product_article,
                                                  sub_subcategory=foreign_key_query)
        create_product = False
        print('Продукт с артикулом %s уже загружн в БД' % existed_product.article_number)
            ...
            for image in images_500:
                ...
                if start_download:
                    ...
                    with open(image_url_to_save, 'wb') as image_file:
                        ...
                        image_file.close()
                        ProductImage.objects.update_or_create(product=created_product, image='products/' + image_title, title=image_title)
            driver.close()
        else:
